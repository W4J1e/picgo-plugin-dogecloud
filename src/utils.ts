...
export function dogecloudExecToken(accessKey:string,secretKey:string,_bucket:string ,force=false,callback){
  try{
      if(fs.existsSync('./token.json')){
        ...
        if(diff >= 7000 || !bucketEqual || force){
          ...
          dogecloudAuth(accessKey,secretKey,_bucket,function(ret){callback(ret)});
        }
      }else{
        dogecloudAuth(accessKey,secretKey,_bucket,function(ret){callback(ret)});
      }
...
function dogecloudAuth(accessKey:string,secretKey:string,_bucket:string,callback ) {//callback回调获值
  //return new Promise(function(resolve,reject){
    ...
    });//回调dogeAnswer，此函数内可以获取data值做后续处理。
